...
class DepthwiseSeparableASPPSwinModule(ASPPModule):
    """Atrous Spatial Pyramid Pooling (ASPP) Module with depthwise separable
    conv."""

    def __init__(self, **kwargs):
        super(DepthwiseSeparableASPPSwinModule, self).__init__(**kwargs)
...
class DepthwiseSeparableASPPSwinCA4Head(ASPPHead):
    ...
    def __init__(self, c1_in_channels, c1_channels, embed_dims, num_heads, feedforward_channels, window_size, **kwargs):
        super(DepthwiseSeparableASPPSwinCA4Head, self).__init__(**kwargs)
        assert c1_in_channels >= 0
        self.aspp_modules = DepthwiseSeparableASPPSwinModule(
        ...
        self.swin_block = SwinBlock(embed_dims, num_heads, feedforward_channels, window_size)
        self.cls_token = nn.Parameter(torch.zeros(1, self.num_classes, self.channels))
        trunc_normal_(self.cls_token, std=.02)

        self.conv_seg = nn.Conv2d(self.channels*self.num_classes, self.out_channels, kernel_size=1)
    ...
    def forward(self, inputs):
        ...
        hw_shape = output.shape[-2:]
        B, C, H, W = x.shape
        output = output.flatten(2, 3).permute(0, 2, 1)
        output = self.swin_block(output, hw_shape)
        output = output.permute(0, 2, 1).view(B, -1, hw_shape[0], hw_shape[1])
        ...
        attn = self.ca_block(output1).mean(1)[:, :, 21:].view(B, 21, H, W).unsqueeze(2) # B, N, C
