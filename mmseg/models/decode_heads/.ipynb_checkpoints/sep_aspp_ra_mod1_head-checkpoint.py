...
class RegionAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        q = self.q(x[:, :self.num_tokens, :]).reshape(B, self.num_tokens, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
...
class DepthwiseSeparableASPPRAMod1Head(ASPPHead):
    ...
    def __init__(self, c1_in_channels, c1_channels, num_tokens, num_heads, num_groups, **kwargs):
        super(DepthwiseSeparableASPPRAMod1Head, self).__init__(**kwargs)
        self.num_tokens = num_tokens
        ...
        self.conv_seg = nn.Conv2d(self.channels*num_groups, self.out_channels, kernel_size=1)
    ...
    def forward(self, inputs):
        ...
        attn = self.ra_block(output1).mean(1)[:, :, self.num_tokens:].view(B, self.num_tokens, H, W).reshape(B, self.num_groups, self.num_tokens//self.num_groups, H, W).mean(2).unsqueeze(2)  # B, T, 1, H, W
       
        output = output.unsqueeze(1) * attn # B, 1, C, H, W
